...
exports.Register = async function (req, res) {
  ...
  try {
    ...
    if (user.block) {
      const checkBlock = await Block.findOne({
        _id: user.block,
        isDeleted: false,
      });
      if (!checkBlock) {
...
exports.GetUserByAdminId = async function (req, res) {
  ...
  try {
    ...
    if (!checkUser || checkUser == "" || checkUser == null) {
      ...
      const checkBlock = await Block.find({
      ...
      if (!checkBlock || checkBlock == "" || checkBlock == null) {
        ...
        const userblock = await Promise.all(
          checkBlock.map(async (block) => {
